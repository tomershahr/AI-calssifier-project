...
class KNNForest():
    def __init__(self ,train_group, test_group,K,p):
        ...
        self.p = p
        self.K = K
        self.id3 = ID3.ID3(self.train_group, self.test_group)
    ...
    def NtreesCreator(self,examples_list, trees_number):
        ...
        for tree in range(trees_number):
            sample_list = random.sample(examples_list, learning_group_size)
            classifier = self.id3.fit(sample_list, self.id3.features, self.id3.MaxIG, self.id3.findMajority(sample_list))
    ...
    def testGroupCalc(self, forest, test_indexes, test_dict):
        ...
        for test in test_indexes:
            classify = self.predictRegardingKTrees(test, forest,test_dict)
    ...
    def predictRegardingKTrees(self, test, forest,test_dict):
        num_B=0
        nun_M=0
        k_closest_trees_and_distance = self.FindKclosestTrees(forest, test,test_dict)
        tree_list = [tree[0] for tree in k_closest_trees_and_distance]
        for tree in tree_list:
            calssifcation = self.id3.predict(test,tree,test_dict)
            if calssifcation == 'B':
                num_B+=1
            else:
                nun_M+=1
    ...
    #For a given test and forest, returns the K closest trees to the test
    def FindKclosestTrees(self, forest, test,test_dict):
        closest_trees=[]
        for tree in forest:
...
###################################################helpers##############################################################
    def tuningParameters(self):
        N_optional_values = [7, 10, 12, 13, 15, 17, 20]
        p_optional_values = [0.3, 0.4, 0.5, 0.6, 0.65, 0.7]
        accuracy_values = []
...
if __name__ == "__main__":
        ...
        KNNForest = KNNForest(train_group,test_group ,K=9,p=0.5)
